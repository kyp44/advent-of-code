        ...
        pub fn execute<'a, P: Part<'a>>(
            ...
            Ok(match self {
                Step::WalkForward(n) => {
                    ...
                    for _ in 0..*n {
                        let new_position = part.next_space(&position)?;
                        match monkey_map.grid.get(&new_position.point) {
                            Space::Void => panic!(),
                            Space::Open => {
                ...
                Step::TurnLeft => Position::new(pos.point, pos.facing.turn_left()),
                Step::TurnRight => Position::new(pos.point, pos.facing.turn_right()),
            })
        }
    }
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            if let Some(ref pos) = ret {
                match self
                    .path
                    .next()
                    .map(|step| step.execute(&self.part, pos, self.monkey_map))
                    .transpose()
                {
                    Ok(o) => {
