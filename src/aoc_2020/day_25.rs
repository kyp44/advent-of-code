...
mod solution {
    ...
    /// An [`Iterator`] that performs transformations on a number given the
    /// subject number.
    #[derive(new)]
    struct Transform {
        /// The subject number.
    ...
    impl Problem {
        ...
        pub fn solve(&self) -> AocResult<u64> {
            /// This is an internal function for [`Problem::solve`] that
            /// determines the secret loop size given an end key.
